...
function renderInputSource(session, inputSource, inputPose) {
  ...
  // we have a valid gripTransform to transform it with.
  if (inputPose.gripTransform && session.environmentBlendMode == 'opaque') {
    // Render a controller mesh the using the gripTransform.
    let controllerMesh = getControllerMesh(inputSource.renderId);
